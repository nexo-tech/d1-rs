...
async fn test_modern_schema_definition() {
    ...
    let users_table = TableDefinition::new("users")
        ...
        .boolean("is_active")
        .default_true()
        .not_null() // Boolean field!
        .boolean("is_verified")
        .default_false() // Another boolean!
        .integer("age")
        .check("age >= 0")
        .datetime("created_at")
...
async fn test_schema_migration_with_booleans() {
    ...
    let posts_table = TableDefinition::new("posts")
        ...
        .default_false() // Boolean with default false
        .boolean("is_featured")
        .default_false() // Another boolean
        .build();
...
async fn test_schema_builder_fluent_api() {
    ...
    let table = TableDefinition::new("complex_table")
        ...
        .default_true()
        .integer("score")
        .default(DefaultValue::Integer(100))
        .check("score >= 0")
        .datetime("created_at")
        .default(DefaultValue::CurrentTimestamp)
        .build();
...
async fn test_foreign_key_constraints() {
    let comments_table = TableDefinition::new("comments")
        ...
        .auto_increment()
        .integer("post_id")
        .not_null() // Test without foreign keys for now
        .integer("user_id")
        .not_null() // Test without foreign keys for now
        .text("content")
        .not_null()
