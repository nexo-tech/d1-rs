...
async fn test_execute_aggregation_sum() {
    ...
    for (i, (expected_name, expected_total)) in expected_totals.iter().enumerate() {
        if let Value::Object(row) = &rows.rows()[i] {
            ...
            assert_eq!(
                row.get("total_score"),
                Some(&Value::Number((*expected_total).into()))
            );
        }
    }
}
...
async fn test_execute_aggregation_max_min() {
    ...
    for (i, (expected_name, expected_max, expected_min)) in expected_max_min.iter().enumerate() {
        if let Value::Object(row) = &rows.rows()[i] {
            ...
            assert_eq!(
                row.get("max_score"),
                Some(&Value::Number((*expected_max).into()))
            );
            assert_eq!(
                row.get("min_score"),
                Some(&Value::Number((*expected_min).into()))
            );
        }
    }
}
...
async fn test_execute_aggregation_string_functions() {
    ...
    let result = data_migration
        ...
        .await
        .expect("CONCAT aggregation failed");
...
async fn test_execute_aggregation_validation_errors() {
    ...
    let result = data_migration
        .execute_aggregation("test_scores", &[], "total_score", "SUM")
        .await
        .expect("Should handle empty source fields gracefully");
    ...
    let result = data_migration
        .execute_aggregation("test_scores", &["math_score".to_string()], "", "SUM")
        .await
        .expect("Should handle empty target field gracefully");
...
async fn test_execute_value_mapping_status_codes() {
    ...
    for (i, (expected_username, expected_old, expected_new)) in expected_mappings.iter().enumerate()
    {
        if let Value::Object(row) = &rows.rows()[i] {
            ...
            assert_eq!(
                row.get("old_status"),
                Some(&Value::String(expected_old.to_string()))
            );
            assert_eq!(
                row.get("new_status"),
                Some(&Value::String(expected_new.to_string()))
            );
        }
    }
}
...
async fn test_execute_value_mapping_with_default() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Value mapping with default failed");
...
async fn test_execute_value_mapping_no_default() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Value mapping without default failed");
...
async fn test_execute_value_mapping_validation_errors() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Should handle empty mapping gracefully");
...
async fn test_execute_value_mapping_sql_injection_safety() {
    ...
    let result = data_migration
        ...
        .await
        .expect("SQL injection safety test failed");
...
async fn test_execute_format_transformation_date_formats() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Date format transformation failed");
...
async fn test_execute_format_transformation_number_formats() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Number format transformation failed");
...
async fn test_execute_format_transformation_phone_formats() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Phone format transformation failed");
...
async fn test_execute_format_transformation_currency_formats() {
    ...
    let result = data_migration
        ...
        .await
        .expect("Currency format transformation failed");
    ...
    if let Value::Object(row) = &rows.rows()[0] {
        ...
        assert_eq!(
            row.get("new_currency"),
            Some(&Value::String("1234.56".to_string()))
        );
    }
}
...
async fn test_execute_format_transformation_case_transformations() {
    ...
    let result = data_migration
        ...
        .await
        .expect("UPPER case transformation failed");
