...
async fn test_column_rename_detection_high_similarity() {
    let strategies = SmartMigrationStrategies::new().with_rename_threshold(0.7);
...
async fn test_column_rename_detection_type_compatibility() {
    let strategies = SmartMigrationStrategies::new().with_rename_threshold(0.5);
...
async fn test_column_rename_detection_multiple_candidates() {
    let strategies = SmartMigrationStrategies::new().with_rename_threshold(0.6);
...
async fn test_data_migration_type_conversion() {
    ...
    let data_migration = strategies
        .plan_column_type_migration("users", &old_def, &new_def)
        .unwrap();

    assert_eq!(
        data_migration.migration_type,
        DataMigrationType::TypeConversion
    );
    ...
    match data_migration.transformation {
        TransformationStrategy::TypeConversion {
            conversion_type, ..
        } => {
...
async fn test_type_conversion_strategies() {
    ...
    // Integer to Text
    let transformation = strategies
        .determine_type_conversion("INTEGER", "TEXT")
        .unwrap();
    ...
    // Text to Integer
    let transformation = strategies
        .determine_type_conversion("TEXT", "INTEGER")
        .unwrap();
    match transformation {
        TransformationStrategy::TypeConversion {
            conversion_type,
    ...
    // Generic conversion
    let transformation = strategies
        .determine_type_conversion("BLOB", "REAL")
        .unwrap();
    match transformation {
        TransformationStrategy::TypeConversion {
            conversion_type, ..
...
async fn test_enhanced_migration_plan_creation() {
    ...
    let enhanced_plan = strategies
        .enhance_migration_plan(original_plan, &diff)
        .unwrap();
